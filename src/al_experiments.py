...
import pickle
import subprocess
from al_experiments.determine_timeout import build_static_timeout, create_instance_wise, quantized_mean_punish, quantized_double_punish, random_timeout, test_static_timeout
...
experiment_configs = [
    ExperimentConfig(
        ...
        instance_selections=[choose_instances_random],  # choose_instances_random, variance_based_selection_1, variance_based_selection_2, highest_rt_selection, lowest_variance, highest_variance, lowest_variances_per_rt, lowest_rt_selection
...
def run_experiment(experiment_config: ExperimentConfig, rt_weight, temp):
    ...
    for index, solver_index in enumerate(random_solver_order):
        ...
        sample_result_after_instances = max(1, int(
        ...
        for breaking_cond in experiment_config.thresh_breaking_conditions:
            ...
            thresholds = experiment_config.determine_thresholds(
                acc_calculator, solver_string, con.number_of_instances, prev_thresholds, con.number_of_reduced_solvers
            )
            ...
            for instance_selection in experiment_config.instance_selections:
                ...
                selector = InstanceSelector(
                    ...
                    experiment_config.determine_thresholds.__name__ == 'static_timeout'
                )
