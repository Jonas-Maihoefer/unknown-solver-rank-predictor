...
def determine_tresholds(
    ...
    for i in range(calc_steps*100):  # will break earlier (if runtime fracion is reached)
        ...
        thresholds, max_acc, min_diff = acc_calculator.add_runtime(
            ...
            mean_par_2_score, runtime_to_add, max_acc, min_diff
        )
        if min_diff == -1:
            break
        if i % 20 == 0:
...
if __name__ == "__main__":
    ...
    plt.plot(random_baseline_whole_instances_runtime_frac, random_baseline_whole_instances_true_acc, marker='x', label="baseline whole instances")
    plt.plot(random_baseline_dynamic_timeout_runtime_frac, random_baseline_dynamic_timeout_true_acc, marker='x', label="baseline dynamic timeout")
